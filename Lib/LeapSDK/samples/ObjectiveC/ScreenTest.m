...
- (void)onFrame:(NSNotification *)notification;
{
    ...
    LeapVector *testPosition = [[LeapVector alloc] initWithX:0 y:300 z:-400];
    LeapVector *testDirection = [LeapVector forward];
    
    LeapScreen *testScreen = [aController.calibratedScreens objectAtIndex:0];
    //LeapVector *intersection = [testScreen intersectFrom:testPosition toward:testDirection normalize:YES clampRatio:1.0];
    LeapVector *intersection = [testScreen intersect:testPosition direction:testDirection normalize:YES clampRatio:.5];
    LeapVector *projection = [testScreen project:testPosition normalize:YES clampRatio:.5];
    NSLog(@"Test intersection %@", intersection);
    NSLog(@"Test projection %@", projection);

    if([[frame pointables] count] != 0)
    {
        LeapPointable *testPointer = [[frame pointables] objectAtIndex:0];
        NSArray *screenlist = [aController calibratedScreens];
        //const LeapScreen *foundScreen = [screenlist closestScreenHitFrom:testPosition toward:testDirection];
        const LeapScreen *foundScreen = [screenlist closestScreen:testPosition];

        NSLog(@"Screen: %d,%d",foundScreen.widthPixels, foundScreen.heightPixels);
    }
    //Testing code end
